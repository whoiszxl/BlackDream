...
```window
./ipfs.exe config --json API.HTTPHeaders.Access-Control-Allow-Methods '["PUT","GET", "POST", "OPTIONS"]'
...
./ipfs.exe config --json API.HTTPHeaders.Access-Control-Allow-Headers '["Authorization"]'
./ipfs.exe config --json API.HTTPHeaders.Access-Control-Expose-Headers '["Location"]'
```